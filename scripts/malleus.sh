...
--*)
  ...
  for arg in "$@"; do
    ...
    --senders-per-node)
      MALLEUS_SENDERS_PER_NODE=$optvalue
      MALLEUS_USE_NODE_TYPES=true
      ;;
    --receivers-per-node)
      MALLEUS_RECEIVERS_PER_NODE=$optvalue
      MALLEUS_USE_NODE_TYPES=true
      ;;
...
if [ "$MALLEUS_USE_NODE_TYPES" = "true" ]; then
...
mvn \
  -Dthreadcount=1 \
  -Dorg.jitsi.malleus.conferences=beer \
  ...
  -Djitsi-meet.tests.toRun=MaxUsersTest \
